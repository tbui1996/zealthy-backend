...
func connect(ctx context.Context, event events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	var createAppointmentRequest request.CreateAppointmentRequest
	if err := json.Unmarshal([]byte(config.Event.Body), &createAppointmentRequest); err != nil {
	...
	return Handler(createAppointmentRequest, deps)
}
