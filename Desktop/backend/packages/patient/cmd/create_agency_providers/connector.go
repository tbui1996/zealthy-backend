...
func connect(ctx context.Context, event events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	var createAgencyProviderRequest request.CreateAgencyProviderRequest
	if err := json.Unmarshal([]byte(config.Event.Body), &createAgencyProviderRequest); err != nil {
		return exception.ErrorMessageApiGatewayV2(http.StatusBadRequest, "unable to parse json body, cannot create agency provider"+err.Error())
	}
	...
	return Handler(createAgencyProviderRequest, deps)
}
